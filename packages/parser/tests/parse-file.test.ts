...
beforeAll(async () => {
  dir = await mkdtemp(join(tmpdir(), 'ccjsonl-'))
  goodPath = join(dir, 'good.jsonl')
...
describe('parseJsonlFile', () => {
  ...
  it('存在しないファイルは Err(FileReadError, ENOENT)', async () => {
    const r = await parseJsonlFile(join(dir, 'nope.jsonl'))
...
describe('parseJsonlFileStream', () => {
  ...
  it('存在しないファイルは反復時に FileReadError を throw する', async () => {
    await expect(async () => {
      ...
      for await (const _ of parseJsonlFileStream(join(dir, 'nope.jsonl'))) {
        /* noop */
      }
